...
def make_synthetic_yf(ticker: str, n_days: int = 1260, seed: int = 0) -> pd.DataFrame:
    """
    Build a synthetic dataframe that LOOKS exactly like what
    yf.download(ticker, period='5y', interval='1d') returns:
      - DatetimeIndex named 'Date'
      - Columns: Open, High, Low, Close, Volume (Adj Close removed by auto_adjust)
      - Business-day frequency
      - Reasonable price/volume dynamics (GBM + vol clustering)
    """
    rng = np.random.default_rng(seed)
    # Asset-class-aware vol/drift so our synthetic looks plausible
    ...
    n = len(dates)  # bdate_range can return slightly different count
    log_rets = rng.normal(drift_per_day, vol_per_day, n)
    close = 100.0 * np.exp(np.cumsum(log_rets))

    # Intraday ranges - high/low straddle close
    ...
    # Volume scales with abs return (vol clustering)
