...
> **Layer 5: Two dashboards available — Google Suite (zero-cost, Colab+Sheets+Apps Script) and Flask (`scripts/dashboard.py`, local GPU).** Google Suite reached feature parity with Flask on 2026-06-06. Both remain fully functional; users can choose based on environment.
...
- **Current state:** All 5 layers ✅ built. **Layer 5: two dashboard options available** — Google Suite (Colab + Sheets + Apps Script, zero-cost, no local GPU required) and Flask (`scripts/dashboard.py`, requires local GPU). Google Suite reached feature parity with Flask on 2026-06-06. Both are fully functional. 15-item QFM enhancement plan ✅ complete. All 4 OOS years (2023-2026) done. Paper trading live since 2026-06-04 (8 trades).
...
### Google Suite Dashboard — Layer 5 (2026-06-04) ✅ AVAILABLE

A second dashboard option, in addition to the Flask dashboard. Zero-cost, browser-based, no local GPU required:
- **Google Colab** — daily compute engine (44-cell notebook, "Run All" each morning)
- **Google Sheets** — persistent data store + fill-confirmation input surface (18 tabs: 10 live + 8 staging)
- **Google Apps Script web app** — 5-tab dashboard SPA (accessible from any device via URL)

**Flask parity achieved 2026-06-06** — 14 misalignments closed across 10 design groups (3 critical, 6 significant, 5 cosmetic). Google Suite is now feature-equivalent to the original Flask dashboard. Spec: `docs/superpowers/specs/2026-06-06-google-suite-dashboard-parity-fixes-design.md`. Plan: `docs/superpowers/plans/2026-06-06-google-suite-dashboard-parity-fixes.md`.
...
**Directory:** `google_suite/`
- `kronos_daily_pipeline.ipynb` — 44-cell Colab notebook (generated by `build_notebook.py`)
- `apps_script/Code.gs` — Apps Script backend (15 functions, 60s cache TTL)
- `apps_script/Index.html` — web app SPA (5 tabs: Dashboard, Trade Ticket, Portfolio, History, Risk)
- `SETUP_GUIDE.md` — click-by-click setup for Google Suite newcomers
- `migrate_to_sheets.py` — ports existing `data/positions/paper_portfolio.json` and `trade_log.csv` to Sheets (fresh-start only — historical data not migrated)

**Original spec:** `docs/superpowers/specs/2026-06-04-google-suite-dashboard-design.md`
**Original plan:** `docs/superpowers/plans/2026-06-04-google-suite-implementation-plan.md` (1,852 lines, all code written)

### Flask Dashboard — Layer 5 (2026-05-21) ✅ AVAILABLE

Local Python + Flask dashboard, requires local GPU. Features built during the original paper-trading setup (still fully functional):

### Flask Dashboard — Improvements Shipped 2026-06-04

All improvements built while paper trading was being set up. Still available; the same patterns informed the Google Suite implementation:
...
5. `google_suite/SETUP_GUIDE.md` — **Google Suite setup** (zero-cost, Colab + Sheets + Apps Script, no local GPU)
6. `docs/dashboard-user-manual.md` — **Flask dashboard setup** (requires local GPU, fully functional alternative)
7. `docs/operations-manual.md` — decision rules reference
8. `docs/user-manual.md` — full methodology, backtest results, and usage instructions
9. `docs/monthly-walkthrough.html` — 21-day simulated month with real trades
10. `docs/superpowers/specs/` — approved design specs (8 active, 5 archived)
11. `docs/superpowers/plans/` — implementation plans (6 active, 14 archived)
12. `docs/superpowers/specs/2026-06-04-google-suite-dashboard-design.md` — **Google Suite spec**
13. `docs/superpowers/plans/2026-06-04-google-suite-implementation-plan.md` — **44-cell Colab + Apps Script plan** (1,852 lines, all code)
14. `docs/superpowers/specs/2026-06-06-google-suite-dashboard-parity-fixes-design.md` — **parity fix spec** (10 design groups, 14 misalignments closed)
15. `docs/superpowers/specs/2026-06-02-real-market-dashboard-design.md` — Flask dashboard spec (original Layer 5 design)
